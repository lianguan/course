...
// Auth handlers moved to admins_auth.go
// Course handlers moved to admins_courses.go
...
// @Success 201 {object} handler.IdResponse
...
func (h *Handler) adminCreateModule(c *gin.Context) {
	...
	c.JSON(http.StatusCreated, handler.IdResponse{moduleId})
}
...
func (h *Handler) adminDeleteModule(c *gin.Context) {
	...
	err = h.services.Modules.Delete(c.Request.Context(), school.ID, uint(id))
	if err != nil {
		handler.NewResponse(c, http.StatusInternalServerError, err.Error())
...
func (h *Handler) adminGetLessons(c *gin.Context) {
	...
	module, err := h.services.Modules.GetWithContent(c.Request.Context(), uint(moduleId))
	if err != nil {
		if errors.Is(err, domain.ErrModuleIsNotAvailable) {
			handler.NewResponse(c, http.StatusForbidden, err.Error())
	...
	c.JSON(http.StatusOK, handler.DataResponse{Data: module.Lessons})
}
...
func (h *Handler) adminCreateLesson(c *gin.Context) {
	...
	c.JSON(http.StatusCreated, handler.IdResponse{lessonId})
}
...
func (h *Handler) adminGetLessonById(c *gin.Context) {
	...
	c.JSON(http.StatusOK, dto.LessonToResponse(lesson))
}
...
func (h *Handler) adminCreatePackage(c *gin.Context) {
	courseID, err := parseIdFromPath(c, "id")
	if err != nil {
		handler.NewResponse(c, http.StatusBadRequest, "empty id param")
...
func (h *Handler) adminGetAllPackages(c *gin.Context) {
	...
	c.JSON(http.StatusOK, handler.DataResponse{Data: toPackagesResponse(pkg)})
}
...
func (h *Handler) adminCreateOffer(c *gin.Context) {
	...
	c.JSON(http.StatusCreated, handler.IdResponse{id})
}
...
type offerResponse struct {
	...
	Price         dto.PriceResponse     `json:"price"`
	PaymentMethod dto.PaymentMethodResp `json:"paymentMethod"`
}
...
func (h *Handler) adminGetAllOffers(c *gin.Context) {
	...
	for i, offer := range offers {
		...
		response[i] = offerResponse{
			...
			Price:         dto.PriceResponse{Value: offer.Price.Value, Currency: offer.Price.Currency},
			PaymentMethod: dto.PaymentMethodResp{UsesProvider: offer.PaymentMethod.UsesProvider, Provider: offer.PaymentMethod.Provider},
...
type createPromocodeInput struct {
	...
	OfferIDs           []uint    `json:"offerIds" binding:"required"`
}
...
// @Accept  json
// @Produce  json
// @Success 200 {object} handler.DataResponse{data=[]domain.PromoCode}
// @Failure 400,404 {object} response
// @Failure 500 {object} response
// @Failure default {object} response
...
func (h *Handler) adminUpdatePromocode(c *gin.Context) {
	...
	var offerIds []uint
	if inp.OfferIDs != nil {
		offerIds, err = stringArrayToUint(inp.OfferIDs)
		if err != nil {
			handler.NewResponse(c, http.StatusBadRequest, err.Error())
...
func (h *Handler) adminGetOrders(c *gin.Context) {
	...
	c.JSON(http.StatusOK, handler.DataResponse{
		Data:  orders,
		Count: count,
	})
}
...
func stringArrayToUint(stringIds []string) ([]uint, error) {
	var err error
