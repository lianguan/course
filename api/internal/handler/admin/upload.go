...
func (h *Handler) adminUploadImage(c *gin.Context) { //nolint:funlen
	...
	if _, ex := imageTypes[contentType]; !ex {
		handler.NewResponse(c, http.StatusBadRequest, "file type is not supported")
	...
	tempFilename := fmt.Sprintf("%d-%s", school.ID, fileHeader.Filename)
...
func (h *Handler) adminUploadVideo(c *gin.Context) { //nolint:funlen
	...
	if !rangeInfo.validSize(maxVideoSize) {
		handler.NewResponse(c, http.StatusBadRequest, "file is bigger than 2 gigabytes")
	...
	if rangeInfo.initialUploadRequest() {
		...
		c.JSON(http.StatusCreated, &uploadVideoResponse{ID: fmt.Sprintf("%d", id)})
