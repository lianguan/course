...
func (h *Handler) studentGetModuleContent(c *gin.Context) {
	...
	c.JSON(http.StatusOK, dto.ModuleContentToResponse(content))
}
...
func (h *Handler) studentGetModuleOffers(c *gin.Context) {
	...
	c.JSON(http.StatusOK, handler.DataResponse{Data: toStudentOffers(offers)})
}
...
func (h *Handler) studentCreateOrder(c *gin.Context) {
	...
	offerId, err := strconv.ParseUint(inp.OfferId, 10, 64)
	if err != nil {
	...
	if inp.PromoId != "" {
		...
		promoIdParsed, err := strconv.ParseUint(inp.PromoId, 10, 64)
		if err != nil {
			newResponse(c, http.StatusBadRequest, "invalid promo id")

			return
		}
		promoId = uint(promoIdParsed)
	}
	...
	id, err := h.services.Orders.Create(c.Request.Context(), studentId, uint(offerId), promoId)
	if err != nil {
		switch err {
	...
	c.JSON(http.StatusOK, createOrderResponse{fmt.Sprintf("%d", id)})
}
...
func (h *Handler) studentGetAccount(c *gin.Context) {
	...
	c.JSON(http.StatusOK, dto.StudentToResponse(student))
}
...
func toSurveyAnswers(answers []surveyAnswer) ([]domain.SurveyAnswer, error) {
	...
	for i := range answers {
		id, err := strconv.ParseUint(answers[i].QuestionID, 10, 64)
		if err != nil {
			return nil, err
		}
