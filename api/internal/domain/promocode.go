...
// UpdatePromoCodeInput 优惠码更新输入（Repository 层使用）
...
func (p *PromoCode) IsExpired(now int64) bool {
	return p.ExpiresAt < now
}

// IsValid 检查优惠码是否有效（未过期且折扣有效）
func (p *PromoCode) IsValid(now int64) bool {
	return !p.IsExpired(now) && p.DiscountPercentage > 0 && p.DiscountPercentage < 100
}
...
// NewPromoCode 创建新优惠码
func NewPromoCode(schoolID uint, code string, discountPercentage int, expiresAt int64, offerIDs []uint) *PromoCode {
	return &PromoCode{
		SchoolID:           schoolID,
		Code:               code,
